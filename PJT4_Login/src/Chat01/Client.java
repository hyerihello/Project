...
import java.io.OutputStream;
	...
	public class Client { //서버가 하나의 클라이언트와 통신하기 위한 기능을 정의
		...
		public void receive() {
				...
				public void run() {
					try {
						while(true) {
							...
							for(Client client : servermain.clients) { //전달받은 메세지를 다른 클라이언트들에게도 통신 가능하게 함
								client.send(message);
							}
						}
					} catch(Exception e) {
			...
			servermain.threadPooL.submit(thread); //Main클레스의 스레드풀 변수에 
					...
					try {
						...
						try {
							System.out.println("[메세지 송신 오류]"
									+ socket.getRemoteSocketAddress()
									+ ": " + Thread.currentThread().getName());
							servermain.clients.remove(Client.this); //메인클레스가 추가된 오류가 뜬 클라이언트를 제거
							socket.close();
						} catch(Exception e2) {
							e2.printStackTrace();
						}
