...
class Tkinter:
    def __init__(self):
        ...
        __sv_super_name_box = StringVar()
        __sv_super_name_box.trace("w", lambda name, index, mode,
                                              buffer=__sv_super_name_box: self.__Entry_value_changed(
            __sv_super_name_box))
        super_name_box = tkinter.Entry(self.window, bg="white", textvariable=__sv_super_name_box,
                                       width=37)  # 이후 크롤링 제목에 들어갈 부분
        super_name_box.place(x=280, y=95, width=300, height=30)

        b1 = tkinter.Button(self.window, text="검색", font=font1,
                            command=self.__Scraping)  # 버튼의 위치, command로 아래 화면이 나오게 연결
        ...
        star_btn = tkinter.Button(self.window, text="별점별", font=font1, command=self.__button1)
        star_btn.place(x=120, y=135, width=110, height=35)
        date_btn = tkinter.Button(self.window, text="날짜별", font=font1, command=self.__button2)
        date_btn.place(x=260, y=135, width=110, height=35)
        sympathy_btn = tkinter.Button(self.window, text="분위기별", font=font1, command=self.button3)
        sympathy_btn.place(x=410, y=135, width=110, height=35)
        sympathy_btn = tkinter.Button(self.window, text="한줄평", font=font1, command=self.__button4)
    ...
    def __button4(self):
        ...
        data = OracleToMongoDB.OracleToMongoDB().line_review()
        for i in range(len(data)):
            self.__trv.insert('', 'end', values=data[i])
    ...
    def __Scraping(self):
        scraping_to_file = ScrapingToFile()
        __title = scraping_to_file.Web_scraping(self.__super_name)
